    ...
    while True:
        ...
        if current_time - last_write_time >= write_interval:
            ...
            if iteration_count >= 20:
                ...
                # Format velocities only if they are not None
                vx_str = f"vx:{last_vx:.2f}" if last_vx is not None else "vx: N/A"
                vy_str = f"y:{last_vy:.2f}" if last_vy is not None else "y: N/A"
                vz_str = f"z:{last_vz:.2f}" if last_vz is not None else "z: N/A"
                
                lcd.write_string(f"{vx_str} {vy_str} {vz_str} ")
                lcd.write_string(f"Fix: {last_fix_type} SV: {last_satellites_visible}")
