    ...
    while True:
        ...
        if current_time - last_write_time >= write_interval:
            ...
            if iteration_count >= 20:
                ...
                lcd.write_string(f"Lat:{last_lat:.3f}Lon:{last_lon:.3f}")
                lcd.write_string(f"R:{last_roll:.2f}")
                lcd.write_string(f"P:{last_pitch:.2f}Yaw:{last_yaw:.2f}SV:{last_satellites_visible} ")
                ret = 1 if ret else 0
                lcd.write_string(f"C:{ret} ")
